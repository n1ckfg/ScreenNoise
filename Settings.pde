  ...
  Settings(String _s) {
    try {
      ...
      for (int i=0;i<settings.data.length;i++) {
        if (settings.data[i].equals("Mode (0=cycle, 1=noise, 2=white, 3=strobe)")) mode = setInt(settings.data[i+1]);
        if (settings.data[i].equals("Mode Delay (seconds)")) modeDelay = setFloat(settings.data[i+1]);
       }
    } 
    catch(Exception e) {
